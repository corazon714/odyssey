<!--
  ...
  C1 MOVED THIS BASELINE, and it is the first of three changes that all push the same way.
  `LEG_JITTER_MAX` 2 -> 1. `rng.nextInt` is inclusive at BOTH ends, so the old `(-1, 2)` drew
  from {-1, 0, 1, 2} with a mean of +0.5 hours per leg, against the symmetric ±1 that
  docs/adr/0014 and docs/adr/0026 both specify. Every route now bills `legCount / 2` fewer
  travel hours: 11 on the 22-leg route, 24 on each of the 48-leg ones. Measured on all 25 corpus
  routes; see the note in docs/adr/0026 for the one place the arithmetic is not exactly
  `legCount / 2`.

  DECOMPOSE BEFORE READING ANY LINE BELOW. The systematic part is that one hour figure — a
  lighter drain, so completion must RISE. Everything else is RE-RANDOMISATION: correcting the
  draw moves every drawn value, so every run walks a different path through the same streams and
  most of the line-by-line diff is a resample. Sub-0.5pp movements are noise, not results.

    Completion rate       43.1% -> 45.6%   (+2.5pp, SYSTEMATIC — the lighter drain)
    Median in-game days       9 -> 8       (systematic — the same route, fewer hours in it)
    Median legs              23 -> 23      (UNMOVED, and it should be: this changes hours, not
                                            leg counts, and the leg count is set by the route)
    failure_gave_up       49.7% -> 47.7%   (-2.0pp — the completion rise, conserved)
    failure_collapsed      7.1% ->  6.6%   (-0.5pp — same direction, at the edge of the band)
    arrival_quiet         43.1% -> 45.5%   (+2.4pp)
    Long-range payoff     18.6% -> 17.6%   (-1.0pp — runs end in fewer DAYS, so fewer scheduled
                                            threads come due inside them; threads 104 -> 105)
    Beat fill rate        27.2% -> 27.4%   (+0.2pp — NOISE)
    Repeat-event rate     66.6% -> 67.0%   (+0.4pp — NOISE)
    Complication rate     60.0% -> 60.2%   (+0.2pp — NOISE, and still on its 60 target)
    Universal picked      27.4% -> 27.3%   (-0.1pp — NOISE)
    Modifier chips/check    6.3 ->  6.3    (unmoved, over 19,948 checks)

  STILL IN BAND, AND DELIBERATELY NOT RE-TUNED. 45.6% is inside engine-spec 6's 30-50%. The
  stop condition this change was run under was "if C1 alone takes pooled completion past ~47%,
  stop" — it did not, so C2 follows.

  NO DRAIN CONSTANT WAS RE-DERIVED HERE, ON PURPOSE. `HOURS_PER_HUNGER_DAMAGE`,
  `HOURS_PER_MORALE` and `FULL_UNTIL` are all hour-denominated and sized against a route set
  that keeps moving until C3. Re-deriving them now guarantees doing it twice; they are verified
  ONCE, at C4, after the route hours stop moving. This is the same trap
  `HOURS_PER_HUNGER_DAMAGE`'s own comment records falling into three times.


  C2 MOVED THIS BASELINE, AND THE MOVE IS COMPOSITION RATHER THAN BEHAVIOUR.
  `acceptByDiversity` now measures overlap in BOTH directions — the reverse check is pairwise per
  accepted route, which is exactly what `verifyPair` measures, so the filter's guarantee and the
  report's metric are finally the same quantity. Verified at 0 post-condition breaches over 1,498
  enumerated pairs against 386 breaching pairs under the old filter.

  THE SCENARIO COUNT FELL 25 -> 23, and that is why every rate here moved. A stricter filter plus
  `load-pack.ts`'s 22-48 leg band left one pair with 3 routes instead of 5. Completion 45.6% ->
  49.5% is NOT a balance change: holding the route mix to the 22 routes common to both builds
  gives 48.8% on BOTH sides — the identical numerator, because they are literally the same runs.
  The corpus lost two routes and the survivors skew easier. **Read the grid marginals, not the
  cell count** (ADR 0038).
  ...
  ── REGENERATED 2026-08-20: THE MONTAGE SPACING CONSTRAINT (ADR 0046) ───────────────────────

  WHAT MOVED, AND WHY. `planLegs` no longer lets montage take three consecutive segments, so
  every corpus route's `legKm` and `montageLegs` changed and this baseline had to move with
  them. The engine's rules did not change; the ROUTES did.

    Completion rate      46.1% -> 46.7%   still inside engine-spec 6's 30-50% band
    Median legs             25 -> 26      runs get further before they end
    Beat fill rate       48.5% -> 48.3%
    Forced-fire legs     24.8% -> 24.3%
    Complication rate    60.5% -> 60.4%   against ATTACH_PERCENT 60, unchanged
    Near-repeat rate     27.3% -> 26.2%   28.18 draws/run against 27.80 — more draws, thinner pool
    Unresolved threads     122 -> 119
    Never-fired events        0 -> 0      unchanged, and it is a gate
    Grid cells sampled  140/140 -> 140/140 — full marginals, 28/28 routes x 5/5 policies

  THE FIXTURE BASELINE DID NOT MOVE, AND THAT NULL RESULT IS EVIDENCE RATHER THAN LUCK.
  `planLegs` is reached only through `route/materialise-route.ts`; the fixture pack reads its
  routes from `engine/src/__tests__/__fixtures__/routes.json` as literal RouteState. So a route
  GENERATION change must print nothing on `--pack=fixture` and must leave every golden run
  untouched — both confirmed, and `pnpm golden:update` was NOT run. `docs/phase-3-closeout.md`
  predicted the opposite ("expect every golden to move") and has been corrected.

  GATE 9 NOW PASSES. At `--runs=280000 --by-route`, no route is below the 3% floor. The two
  routes that failed it read 6.95% (+15.5 SE) and 12.26% (+28.2 SE), from 2.32% and 2.81%.

  AND ONE ROUTE GOT WORSE, WHICH IS NOT HIDDEN HERE: `route.illicit.r1gjd3s6`, the healthy
  comparable every ADR 0044 experiment was measured against, fell 16.51% -> 11.32% and its
  morale-floor share rose 60.39% -> 68.50%. The corpus COMPRESSED — the four Beira-Aktobe
  illicit routes now span 6.95-14.68% where they spanned 2.32-16.51%. The mechanism for that
  regression is NOT established. See ADR 0046 §5.
...
Completion rate             46.7%   (target band 30-50%)
Median legs                    26
...
Forced-fire legs            24.3%   (beat slot or queue due — never gated; caps quiet at 75.7%)
Long-range payoff rate      20.3%   (target 80%)
Beat fill rate              48.3%
Repeat-event rate           66.6%
Near-repeat rate            26.2%   (a redraw inside recency's own 6-event window; 28.18 draws/run — LESS confounded than the line above, NOT unconfounded: subtract a null baseline before reading it)
Complication rate           60.4%   (target 60%)
Modifier chips / check        6.3   (target 3-7, over 21251 checks)
...
Unresolved threads            119

Wall clock                 2038 ms   (1.02 ms/run)
Extrapolated to 20,000     20.4 s   (target <30 s)

## Endings
  ending.failure_gave_up              45.9%
  ending.arrival_quiet                42.1%
  ending.failure_collapsed             7.3%
  ending.arrival_hollow                3.1%
  ending.arrival_triumphant            1.2%
  ending.detained_at_border            0.3%
...
## Choices picked <2%
  ...
  city.the_last_kilometre/spend_the_last_of_it         0.5%
  ...
  transit.the_wrong_carriage/move_before_they_reach_you   1.9%
...
## Resource trajectories (p10/p50/p90 by leg)
  cash     leg5: 993/2267/4166   leg15: 865/2042/3880   leg25: 1253/2191/3616
  health   leg5: 9/10/10   leg15: 5/7/9   leg25: 3/5/8
  morale   leg5: 5/7/9   leg15: 2/6/10   leg25: 2/6/10
